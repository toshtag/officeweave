...
class RecurringEventTest < ActiveSupport::TestCase
  ...
  test "毎週の繰り返しを作る" do
    # 終わりは最初の回からの間隔で指定する。今日からの日数で指定すると、
    # 最初の回が明日であることに結果が左右される。
    series = create_series(frequency: "weekly", repeat_until: first_start + 3.weeks)
  ...
  test "毎日の繰り返しを作る" do
    series = create_series(frequency: "daily", repeat_until: first_start + 2.days)

    assert_equal 3, series.size
  end

  test "毎月の繰り返しを作る" do
    series = create_series(frequency: "monthly", repeat_until: first_start + 2.months)
  ...
  test "繰り返しの終わりを超えない" do
    repeat_until = first_start + 10.days
  ...
  test "各回は同じ参加者を持つ" do
    series = create_series(frequency: "daily", repeat_until: first_start + 1.day,
  ...
  test "上限を超える回数は作らない" do
    event = build_event
    far = (Event::Recurrence::MAXIMUM_OCCURRENCES + 5).days.from_now.to_date
    result = Event::Recurrence.new(event, frequency: "daily", repeat_until: far)
  ...
  test "このあとの回をまとめて消せる" do
    series = create_series(frequency: "daily", repeat_until: first_start + 3.days)
  ...
  private
    # 最初の回の開始日。
    def first_start = 1.day.from_now.to_date
    ...
    def create_series(frequency:, repeat_until:, attributes: {}, participants: [])
      ...
      return [ event ] if event.reload.series_id.nil?

      Event.where(series_id: event.series_id).chronological.to_a
    end
end
