...
セルフホストでは、バックアップの取得と保管は導入した組織の責任になります。
この製品は取得と復元の手立てを用意しますが、保管先の用意、定期実行の設定、
保管したものが復元できるかの確認は、運用する側で行ってください。

最短の導線です。

```text
取る        script/production_backup（下記「2. 取得」）
戻す        FORCE=1 script/production_restore <書庫>（「3. 復元」）
別のホストへ  「5. 別のホストへの復元」
```

以下は、その詳細と、失敗したときの挙動です。
