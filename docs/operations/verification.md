...
入口の画面で、未読の印と件数が正しく出ることを確かめる。

```text
1. 並べたお知らせのうち、読んでいないものに未読の印が付く
2. 見出しの件数は、並べた 5 件を超える未読も含めた総数になる
3. お知らせを開くと、次に入口を開いたとき印が消え、件数が 1 減る
4. 未読が無ければ件数を表示しない
```

未読の判定は、画面へ並べる分だけで行う。参照できる未読を全件取り出すと、
5 件の表示のために蓄積した全件を読むことになる。お知らせを読まない利用者
ほど重くなる。

件数は数え上げで求める。取り出した集合の大きさから求めると、数だけを表示
するために識別子を全件持つことになる。

お知らせの一覧は同じ形にしない。そちらは表示する分と読み込む分が一致して
おり、この問題に当たらない。一覧の件数の上限はロードマップ R4 で扱う。

読み込む行の数は `test/controllers/home_unread_test.rb` が押さえている。
