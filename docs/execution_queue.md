...
見ており、系列そのものが上がっていることを取りこぼした。R10-T4 で扱う。
PostgreSQL、Chromium、スクリプトが起動する道具は、系列を含めて確認した結果、
上げる先が無い。

R10-T4 は言語処理系のメジャー更新であり、[技術構成](development/tech_stack.md#メジャー更新)
が定めるとおり、実装より先に技術構成の対象系列を改める。
...
| R10-T4 | `r10/t4-ruby-update`    | —        | 言語処理系の系列を 4.0 へ上げる          | 進行中 |
