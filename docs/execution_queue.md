...
```text
現在の Phase: R10（R11 の完了後に足した追補）
直近完了 Task: R10-T7
進行中:        なし
次に実行:      なし（ロードマップの残タスクは無い）
```
...
[ロードマップ](roadmap.md) の R3 を、単独で検証できる単位へ分解した。
機能の追加であるため、GitHub Issue では管理しない。

部門の CSV 取込を先に置く。ロードマップがブロッキング前提として挙げており、
他のタスクへ依存しない。

承認は、多段、経路の記録、代理の順に進める。多段を先に入れないと、
記録すべき経路そのものが存在しない。代理は、経路が決まっている前提で
「誰の代わりに決裁したか」を足すものである。

暦は、予約の変更、参加者、繰り返しの順に進める。繰り返しは予定の複製で
あり、参加者の扱いが決まってから足す。
...
| R3-T4 | `r3/t4-delegated-approval`       | —        | 代理承認を用意する                 | 完了  |
| R3-T5 | `r3/t5-reservation-change`       | —        | 予約の変更を用意する                | 完了  |
| R3-T6 | `r3/t6-event-participants`       | —        | 予定の参加者を用意する               | 完了  |
| R3-T7 | `r3/t7-recurring-events`         | —        | 繰り返し予定を用意する               | 完了  |

## R4 規模・操作性

[ロードマップ](roadmap.md) の R4 を、単独で検証できる単位へ分解した。
機能の追加であるため、GitHub Issue では管理しない。

ページネーションを先に置く。絞り込みは、絞った結果を並べる場所が
決まってから足す。順序を逆にすると、絞り込みの結果が全件表示のままになる。

| Task  | ブランチ                          | 対応 Issue | 内容                        | 状態  |
| ----- | ----------------------------- | -------- | ------------------------- | --- |
| R4-T1 | `r4/t1-list-pagination`       | —        | 一覧のページ送りを共通化する            | 完了  |
| R4-T2 | `r4/t2-list-filters`          | —        | 一覧の絞り込みを用意する              | 完了  |
| R4-T3 | `r4/t3-api-paging`            | —        | API のページングとレート制限を設ける      | 完了  |
| R4-T4 | `r4/t4-search-scope`          | —        | 検索の対象を広げる                 | 完了  |

## R5 品質・セキュリティ

[ロードマップ](roadmap.md) の R5 を、単独で検証できる単位へ分解した。
機能の追加であるため、GitHub Issue では管理しない。

配信する内容の制限を先に置く。R0 のセキュリティレビューで不在を確認して
おり、R5 へ計上したまま残っている項目である。

次に、供給の経路に関わるもの（参照の固定、部品表、コンテナの検査）を
まとめて扱う。いずれも CI の構成であり、製品の振る舞いを変えない。

検査の追加（アクセシビリティ、実ブラウザー、性能）は最後に置く。
実行環境へ道具を持ち込むため、前の 4 件より前提が重い。

このうち T5 と T6 は、登録した順とは逆に T6 から行う。アクセシビリティの
検査は、規格の判定を自分で書くのではなく既存の実装へ任せる。その実装は
ブラウザーの中で動くため、実ブラウザーの実行方法が先に要る。

性能・負荷の検証は 2 つに分けた。前提と結果の性質が違う。

- 問い合わせの回数は、データ量を変えても同じ結果になる。追加の道具を要さず、
  一括検証へ入れられる
- 応答の速さは、動かす機械によって変わる。合否には使えず、報告として残す。
  データを積む手順と負荷をかける道具が要る

同じ 1 つのタスクにすると、合否に使える検証と使えない検証が混ざる。

| Task  | ブランチ                              | 対応 Issue | 内容                       | 状態  |
| ----- | --------------------------------- | -------- | ------------------------ | --- |
| R5-T1 | `r5/t1-content-security-policy`   | —        | 配信する内容の出所を制限する           | 完了  |
...
| R5-T8 | `r5/t8-load-measurement`          | —        | 負荷をかけて応答を測る              | 完了  |
...
[ロードマップ](roadmap.md) の R10 を、単独で検証できる単位へ分解した。
機能の追加ではないため、GitHub Issue では管理しない。

分ける根拠は、確かめる手段が違うことである。

- gem は、一括検証で確かめられる
- action は、継続的インテグレーションを実行しなければ確かめられない
- 依存を解決する道具は、両方のコンテナの組み立てからやり直す

依存を解決する道具は最後に置く。先に上げると、gem を上げた差分へ、道具の
系列が変わったことによる書き換えが混ざり、どちらが原因かを切り分けられない。

コンテナのイメージのうち、Ruby は 4 系が出ている。R10-T1 では 3.4 系の中だけを
見ており、系列そのものが上がっていることを取りこぼした。R10-T4 で確かめ直す。
PostgreSQL、Chromium、スクリプトが起動する道具は、系列を含めて確認した結果、
上げる先が無い。

| Task   | ブランチ                       | 対応 Issue | 内容                          | 状態  |
| ------ | -------------------------- | -------- | --------------------------- | --- |
| R10-T1 | `r10/t1-gem-update`        | —        | gem を入手できる最新の版へ上げる          | 完了  |
| R10-T2 | `r10/t2-action-update`     | —        | GitHub Actions の参照を最新の版へ上げる | 完了  |
| R10-T3 | `r10/t3-bundler-update`    | —        | 依存を解決する道具の系列を上げる            | 完了  |
| R10-T4 | `r10/t4-ruby-series-review` | —        | 言語処理系の系列を確かめる               | 完了  |
| R10-T5 | `r10/t5-version-source`    | —        | 実行環境の版の定義場所を整理する            | 完了  |
| R10-T6 | `r10/t6-ruby-version-format` | —      | 版の書き方を定義場所でそろえる              | 完了  |
| R10-T7 | `r10/t7-ruby-series-rationale` | —    | 系列を採らない理由の記述を実態へそろえる         | 完了  |

R10-T4 は、上げるのではなく上げないことを選んだ。移って得るものが無く、
配布物に報告の対象となる版が含まれる分だけ払うものがあるためである。
判断の根拠と、上げ直す条件は [技術構成](development/tech_stack.md#ruby-40-を今は採らない理由) に置く。

R10-T5 を最後に置く予定だった。T1 から T4 で、同じ値を何か所で直すことになるか
が実際に分かったためである。

R10-T6 は R11 の完了後に足した。T5 で入れた検査が、版を比べる前に書き方の
違いを吸収していた。同じ値が 2 通りで書かれたままであり、T5 の「食い違ったら
気づける」が成り立っていなかった。R11 の対象とは別であるため、R10 へ戻して
番号を続ける。

R10-T7 も同じ経緯である。T4 で書いた「採らない理由」が、検査器の重大度の表示
だけを根拠にしており、危険度を実際より大きく書いていた。判断そのものは変わら
ないが、根拠が違う。読んだ者が同じ誤解をしないよう書き直す。

## R11 コンテナ実行環境

[ロードマップ](roadmap.md) の R11 を、単独で検証できる単位へ分解した。
機能の追加ではないため、GitHub Issue では管理しない。

分ける根拠は、確かめる手段が違うことである。

- 名称は `docker compose config` の出力で確かめられる
- 組み立ての速さは、同じ変更を与えて組み立て直した時間で比べる
- 起動の速さは、停止した状態から健全になるまでの時間で比べる
- 残るものは、実行の前後で一覧の差を取る

名称を最初に置く。イメージを 1 つにまとめると、その名前をどう決めるかを
先に固める必要がある。後から変えると、組み立ての計測をやり直すことになる。

R11-T5 は T1 から T4 の後に置く。両方の構成へ何を入れたかが出そろってから
でないと、対称であるべき範囲を決められない。

| Task   | ブランチ                    | 対応 Issue | 内容                     | 状態  |
| ------ | ----------------------- | -------- | ---------------------- | --- |
| R11-T1 | `r11/t1-container-names` | —        | 表示される名称と一覧の項目数を減らす     | 完了  |
| R11-T2 | `r11/t2-image-build`    | —        | 組み立ての無駄を取り除く           | 完了  |
| R11-T3 | `r11/t3-startup-wait`   | —        | 起動の待ち時間を詰める            | 完了  |
| R11-T4 | `r11/t4-leftovers`      | —        | 立ち上げるたびに残るものを無くす       | 完了  |
| R11-T5 | `r11/t5-configuration-symmetry` | — | 片方だけを直した状態に気づけるようにする | 完了  |
