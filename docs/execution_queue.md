...
R0-T26 の後に行った性能レビューで登録した 6 件を、1 回の操作あたりに増える
問い合わせの多い順に処理した。R0-T33 ではコードを触らず、結果の確認と状態の
反映だけを行う。

```text
#119  お知らせの公開が受け手の人数に比例した問い合わせを行う      CLOSED
#121  部門の階層表示が上位を 1 段ずつ問い合わせる                CLOSED
#120  利用者一覧の主たる所属で先読みが働かない                    CLOSED
#122  入口が表示しない分まで未読を全件読み込む                    CLOSED
#123  認証のたびに最終活動時刻の書き込みが発生する                CLOSED
#124  お知らせ一覧の下書きと公開待ちで作成者が 1 件ずつ引かれる   CLOSED
```

実測した改善は次のとおりである。いずれも表示する内容は変えていない。

```text
受け手 30 名の公開              69 queries ->  8 queries
部門 120 件・深さ 6 の一覧     202 queries ->  2 queries
利用者 31 名の一覧              64 queries ->  3 queries
お知らせ 300 件の入口          305 行     -> 11 行
画面の認証 1 回                  3 queries ->  2 queries
token の認証 1 回                3 queries ->  1 query
下書き 12 件の一覧              24 queries -> 14 queries
```

```text
Issue          登録した 40 件すべてが CLOSED
実行キュー      R0-T0 から R0-T33 まで、未着手なし
全テスト        1047 runs, 3910 assertions, 0 failures, 0 errors
bin/rubocop     278 files inspected, no offenses detected
bin/brakeman    No warnings found
bin/verify      成功
CI              6 件の PR すべてで全ジョブ成功
文書            CHANGELOG、検証手順、実行キューの相互参照に欠落なし
```

#### 測り方について

性能の退行は、処理が出す問い合わせの件数が入力の大きさに比例して増える形で
現れる。実行の速さで確かめると、実行環境の速さに左右されて間欠的に失敗する。
R0-T15B と R0-T17C で扱ったのと同じ性質である。

そこで、出した問い合わせの件数と読み込んだ行の数を数える形とした。
期待値へ絶対の件数は書かない。入力の大きさを変えて 2 回数え、増えないことを
確かめる。手立ては `test/test_helpers/query_count_test_helper.rb` にある。

数える前に問い合わせのキャッシュを空にする。キャッシュは運用では要求ごとに
作り直されるが、テストでは同じ接続を使い続けるため、2 回目がキャッシュから
返って比較が成立しない。

#### 範囲外として残した確認

一覧のページネーションは扱っていない。ロードマップ R4 に計上済みの項目で
あるため、Issue へは登録していない。本書とロードマップで管理する範囲と、
Issue で管理する範囲を混ぜない。R0-T27 から R0-T32 は、表示する件数を
変えずに減らせる問い合わせだけを対象とした。

したがって、一覧が返す件数そのものは今も無制限である。記録が増え続ければ
一覧は重くなる。**R4 の着手条件ではないが、運用の規模が大きくなる場合は
ここが最初の制約になる。**

CSV の取り込みは行ごとに問い合わせを出す。誤りの行番号を返すために行単位で
処理しており、管理者が明示的に実行する操作であるため、対象としていない。
...
| Task  | ブランチ                                  | 対応 Issue                                                  | 内容                     | 状態  |
| ----- | ------------------------------------- | --------------------------------------------------------- | ---------------------- | --- |
| R6-T1 | `r6/t1-document-implementation-drift` | [#136](https://github.com/toshtag/OfficeWeave/issues/136) | 規約と品質基盤の文書を実装へそろえる     | 進行中 |
| R6-T2 | `r6/t2-environment-documentation`     | [#133](https://github.com/toshtag/OfficeWeave/issues/133) | 環境変数の説明の正本を 1 か所へ定める   | 未着手 |
| R6-T3 | `r6/t3-form-field-helper`             | [#132](https://github.com/toshtag/OfficeWeave/issues/132) | 入力欄の組み立てを 1 か所へまとめる    | 未着手 |
| R6-T4 | `r6/t4-test-layers`                   | [#137](https://github.com/toshtag/OfficeWeave/issues/137) | テストの置き場所を方針の層へそろえる     | 未着手 |
...
- R6-T1 を先頭へ置いた。文書に従うと現状と違うものを作る状態であり、
  後続のどのタスクでも、規約を参照した時点で誤りが混ざる。
- R6-T2 を次に置いた。配布用の構成が必須とする変数が雛形と設定の文書の
  両方から抜けており、README の手順どおりでは起動しない。
  R6 の中で、利用者の手元に実害が出るのはこれだけである。
- R6-T3 は唯一コードを触る。文書側の食い違いを片付けてから入る。
- R6-T4 はテストの移動を伴う。R6-T3 の変更と競合しないよう、後段へ置いた。
- R6-T5 と R6-T6 は記録の整理であり、他のタスクが総合検証と本書へ書き足す分を
  取り込んでから行う。R6-T6 を最後にしたのは、R6-T1 から R6-T5 が本書の
  状態を更新するためである。
