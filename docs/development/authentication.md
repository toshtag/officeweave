...
id_token から読む属性は次の 4 つだけとする。ほかの属性は読まない。
読む属性が増えるほど、認可サーバー側の設定と、この製品の振る舞いの
結び付きが増える。

| 属性             | 必須 | 使い道                                   |
| ---------------- | ---- | ---------------------------------------- |
| `sub`            | 必須 | 認可サーバー側の識別子。突き合わせには使わない |
| `email`          | 必須 | この製品の利用者を引き当てる              |
| `email_verified` | 任意 | 記載があり `false` のものを拒む            |
| `nonce`          | 必須 | 要求と応答の対応を確かめる                 |

読まない属性のうち、扱いを誤りやすいものを挙げる。

```text
name                表示名は、この製品側の登録を正本とする
preferred_username  引き当てには使わない。可変であり、重複し得る
groups / roles      権限はこの製品側だけで決める
picture             取り込まない。外部への取得を要求ごとに起こさない
```

`sub` は必須として検証するが、利用者の引き当てには使わない。引き当ては
メールアドレスで行う（5.5）。`sub` で引き当てる形にすると、認可サーバーを
入れ替えたときに、この製品側の全利用者を結び付け直すことになる。

属性が足りない場合の扱いは次のとおりである。

```text
sub が無い             id_token の検証で拒む
email が無い           id_token の検証で拒む
email_verified が無い  確かめられないものとして、そのまま通す
email_verified が false 拒む
nonce が一致しない      拒む
```

対応は `test/models/authentication/oidc_claim_contract_test.rb` が固定する。
