...
本書と同じ内容を、機械が読む形でも持つ（[機能到達度の一覧](capability_registry.yml)）。
件数、識別子、状態、入口、証拠、条件の判定を、そちらが持つ。両方を人が別々に
直すと必ず食い違うため、`test/configuration/completion_registry_test.rb` が
突き合わせる。片方だけを直した変更は、そこで落ちる。

Core の未達と、採用済みの Suite 拡張が重なる項目は、機械が読む一覧で持ち主を
明示する。重なりを Core の完成条件へ引き込むと、Suite を実装しないという
約束と両立しない。
...
release_gate         1  release.production_readiness（定義）
  版ごとの評価       1  passed した版は 1
```

段階は Core・Suite・Extended の 3 つである。範囲外は第 4 の段階ではない。

複数の Core 機能にまたがる横断の課題は、いずれも解消した。

条件 7 は解消した。増え続ける記録には、それぞれ削除の方針がある。期限切れの
ログイン、終わったジョブ、監査記録、試行の数え上げ、通知、送信の記録、
運用の異常を知らせた記録である（`config/recurring.yml`）。既定はいずれも
「消さない」とし、日数を指定した場合だけ消す。

条件 3 も解消した。通知は宛先と発生の組で 1 件だけ作られる。
...
機能ごとの未達も解消した。権限の粒度、一覧の上限、選択欄の描画量、
データベースの不変条件、想定規模での実測、操作の文書である。どこで
どう解消したかは、それぞれの節に書いた。

本番準備は機能ではなく版に対して判定する。機能がすべて `complete` に
なったことと、版として本番へ出せることは別である。判定は版ごとの評価が
持ち、根拠は `docs/releases/` の実測の記録にある。
...
条件 5 は非該当。利用者・部門・設備を選ばせる欄を持たない。
...
代理で決裁できるのは、委任元も利用でき、その段の担当を現在も持つ場合に限る。
委任は担当を移さない。委任元を無効にすれば、その立場は委任先からも消える。

範囲の解き方は `app/models/delegated_approval.rb` へ 1 つだけ置き、決裁の立場、
承認待ちの一覧、申請の参照、通知の対象がいずれもそれを通る。参照を通さないと、
一覧には出るのに画面から開けない。部門を指定しない段（管理者が担当する段）の
委任も同じ経路で扱う。
...
条件 7 は満たす。`NOTIFICATION_RETENTION_DAYS` を指定した場合、その日数より
古い通知を毎日消す。
...
条件 3 は満たす。組織につき 1 回だけ積み、本文に出来事と発生の単位を入れる。
配送は at-least-once とするため、受け取る側がその組で二度届いた分を捨てられる。

条件 7 は満たす。`WEBHOOK_DELIVERY_RETENTION_DAYS` を指定した場合、その日数より
古い送信の記録を毎日消す。

出来事ごとの形と版は `app/models/webhook_payload.rb` が持ち、
`test/models/webhook_payload_contract_test.rb` が固定する。予定の招待にも
送る経路を足したため、一覧に挙げた 5 つはすべて送られる。
...
条件 3 は満たす。異常の名前と状態の組を発生とし、記録の作成そのもので判断する。
同じ異常が続くあいだは送り直さず、一定の間隔を過ぎたときだけ知らせ直す。

条件 7 は満たす。`OPERATIONAL_ALERT_RETENTION_DAYS` を指定した場合、その日数より
古い記録を毎日消す。
