...
経路  利用者から見た入口。経路を持たない機能は「入口」を書く
...
`complete` は現在 1 件である。
Core の大半が `partial` で止まる理由は、次の 2 つがどの機能でも満たされていないためである。

- 条件 2: 更新の経路に、監査へ記録するかしないかの割り当てが無い
- 条件 7: 増え続ける記録のうち、保持の方針があるのは監査記録だけである

この 2 つは機能ごとに直すものではない。先に横断して片付ける。
...
模型  なし
検証  なし
文書  docs/development/release.md（手順のみ）
```

未達:

- `docs/releases/` が存在しない。実行結果を版へ結び付ける置き場が無い
...
| 狭い画面での案内        | 320 px と拡大表示で主要機能へ到達する        |
| 予定の暦表示          | 月・週・日の表示                      |
