...
申請の版を署名した値を画面が持ち帰り、占有したあとに突き合わせる
（`app/models/request_decision_token.rb`）。署名するため、先の段や別の利用者の
値を自分で作れない。差し戻して再提出し、同じ段の位置へ戻った場合も、
版が変わるため前の提出のときの要求は通らない。承認済み、差し戻し済み、
取り下げ済みへ進んだあとの古い要求も、同じく競合として拒む。

立場は、利用者が有効であること、申請と同じ組織であること、その段の担当または
委任先であることを、占有したあとに確かめる。制御部の参照範囲に頼らない。

通知と外部への送信は、いちばん外側の確定のあとにだけ行う。
