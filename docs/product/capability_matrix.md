...
立場の判定、待っている段の確定、期待した段との一致、状態の変更、段の印、履歴、
代理元、監査を、同じ行の占有のなかで確定する（`app/models/request_decision.rb`）。
通知と外部への送信は確定したあとに出す。

期待する段は位置で照らす。段 2 の担当が、段 1 を待っているあいだに段 2 を
指した要求は、占有した時点で段 2 になっていれば成立する。どの段も、その段の
担当だけが通すため、段を飛ばす形にはならない。要求が作られた時点の版まで
照らすには、行に版を持たせる必要がある。今は持たせていない。
...
- 2: 更新の経路に対して、監査へ記録するかしないかの割り当てが網羅されていない。
  ...
  取り下げ、中間の段の承認、申請の種別、自分の設定には割り当てが無い。
