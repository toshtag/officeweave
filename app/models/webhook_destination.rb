...
class WebhookDestination
  ...
  #
  # 範囲を足すかどうかは、IANA の special-purpose address registry で
  # 判断する。globally reachable でないものは、外部の宛先になり得ない。
  # globally reachable であっても、IPv4 を内包する形式と、機器や網の
  # 内側を指す用途のものは足す。registry は実行時には読まない。
  #
  # 一覧では理由を書けないため、ここに残す。
  #
  #   fec0::/10                     廃止済みの site-local。既存の網に残り得る
  #   64:ff9b::/96 64:ff9b:1::/48   IPv4 との変換。下位に IPv4 を内包する
  #   2002::/16 192.88.99.0/24      6to4 とその中継。同じく IPv4 を内包する
  #   2001::/23                     IETF が用途を定めた塊。Teredo もこの中にある
  #   100::/64 3fff::/20 5f00::/16  破棄専用、文書例示、経路制御の識別子
  #
  # 内包した IPv4 を取り出して判定し直す形は採らない。取り出す位置は
  # 接頭辞の長さで変わり、誤ると迂回がそのまま残る。範囲ごと拒否する。
  # 変換を経て外部へ出る宛先は、許可リストで origin ごとに許す。
  BLOCKED_RANGES = %w[
    0.0.0.0/8
    10.0.0.0/8
    100.64.0.0/10
